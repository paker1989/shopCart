...
function mergeChunks(fileName, chunks, callback) {
    let chunkPaths = chunks.map(function (name) {
        return path.join(__dirname, '../assets/blog',
            _BLOG_LOCAL_CONFIG._CHUNK_FILE_TEMP, name)
    });

    // 采用Stream方式合并
    let targetStream = fs.createWriteStream(path.join(__dirname, '../assets/blog',
...
const saveSimpleBlog = (req, res) => {
    ...
    fs.writeFile(path.join(__dirname, '../assets/blog',
...
const finalizeChunckFile = (req, res) => {
    ...
    if (finalize) {
        fs.readdir(path.join(__dirname, '../assets/blog',
